...
def bitproof(message, commitment, state):
    ...
    if message == 0:
        ...
        x1 = (
            pow(state.h, y1, state.p)
            * pow(commitment.c
                  * pow(state.g, -1, state.p), -e1, state.p)
            % state.p
            )
...
def verify(c, p_state, bp_state):
    ...
    c_c = (
        bp_state.x0 * pow(c, bp_state.e0, p_state.p) % p_state.p
           ==
        pow(p_state.h, bp_state.y0, p_state.p)
        )
    c_d = (
        (bp_state.e1 + bp_state.e0) % p_state.p
        ==
        bp_state.e
        )
